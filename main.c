...
//Составление массива из дерева в правильном порядке
...
int main(int argc,char *argv[])
{
    ...
    switch (inputinput){
        case 1:{
            printf("Вы выбрали ввод вручную\n");
        ...
        case 3:{
            printf("Выбрано случайное заполнение\n");
            massive = (unsigned long int *)malloc(s * sizeof(unsigned long int));
            for ( i = 0; i < s; i++) {
    ...
    switch (inputsortway){
        ...
        case 1:{
            ...
            printf("\nПримерное время сортировки:  %lf сек\n",T);
            printf("\nДвижений по массиву: [%lu]", moves);
            printf("\nЧисло сравнений [%lu]\n", comp);
            moves=0;
            comp=0;
            T = 0;
        ...
        case 3:{
            ...
            treeIntoArray(root,pack); //Собираю дерево обратно в массив
            Tend=clock();
            T=(Tend-Tstart)/(double)CLOCKS_PER_SEC;
            printf("\nПримерное время сортировки:  %lf сек\n",T);
            printf("\nДвижений по массиву: [%lu]", moves);
            printf("\nЧисло сравнений [%lu]", comp);
            moves=0;
            comp=0;
            T = 0;
            ...
            if (min) {
                ...
                printf("\nПримерное время поиска:  %lf сек\n",T);
                printf("\nДвижений по дереву: [%lu]", moves);
                printf("\nЧисло сравнений [%lu]\n", comp);
                moves=0;
                comp=0;
                T = 0;
        ...
        case 4:{
            ...
            Tend=clock();
            T=(Tend-Tstart)/(double)CLOCKS_PER_SEC;
            printf("\nПримерное время сортировки:  %lf сек",T);
            printf("\nДвижений по массиву: [%lu]", moves);
            printf("\nЧисло сравнений [%lu]\n", comp);
            moves=0;
            comp=0;
    ...
    printf ( "Хотите увидеть отсортированный массив?:\n 1 Да\n 2 Нет\n");
    scanf("%d",&see);
   switch (see){
       ...
       printf("Введите ключ для поиска:\n");
        ...
        switch(inputsearchway){
            ...
            case 2:
            {
                ...
                if (value != -1) {
                    printf("Индекс искомого элемента %ld\n", value);
                    printf("Примерное время поиска:  %lf сек\n",T);
                    printf("Движений по массиву: [%lu]\n", moves);
                    printf("Число сравнений [%lu]", comp);
                    moves=0;
                    comp=0;
                    T = 0;
                }
